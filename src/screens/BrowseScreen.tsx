...
export function BrowseScreen({ useAbhangasHook, useFavoritesHook }: Props) {
  const navigation = useNavigation<NativeStackNavigationProp<HomeStackParamList>>();
  const route = useRoute<RouteProp<HomeStackParamList, 'Browse'>>();
  const saintParam = route.params?.saint;
  ...
  // Apply saint filter from navigation param
  useEffect(() => {
    if (saintParam) {
      filterBySaint(saintParam);
    } else {
      selectAllSaints();
    }
    // Reset to all on unmount
    return () => { selectAllSaints(); };
  }, [saintParam]);
